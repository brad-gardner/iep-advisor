...
export function FinalizeSection({
  ...
  const handleConfirm = async (effectiveDate: string | null) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        setIsOpen(false);
        show({
          message: `Finalized v${res.data.versionNumber}`,
          variant: "success",
        });
  ...
  const closeDialog = () => {
    // Don't let Esc/backdrop dismiss the dialog mid-finalize — wait for the
    // parent-lifted submit to settle.
  ...
  return (
    <div className="space-y-3">
      ...
      {finalized && (
        <Notice
          variant="success"
          title={`Finalized v${finalized.versionNumber}`}
        >
